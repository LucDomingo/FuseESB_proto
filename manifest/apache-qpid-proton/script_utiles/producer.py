...
def get_options():
    parser = optparse.OptionParser(usage="usage: %prog [options]",
        description="Envoi message sur broker")
    parser.add_option("-u", "--url", action="store", default="amqp://localhost:5672",
        help="url amqp (default %default)")
    parser.add_option("-t", "--topic", action="store", default="a/topic",
        help="topic cible (default %default)")
    parser.add_option("-q", "--queue", action="store", default="a/topic",
        help="queue cible (default %default)")
    parser.add_option("-c", "--payload", action="store", default="a/topic",
        help="payload message (default %default)")
    parser.add_option("-m", "--messages", type="int", default=100,
        help="nombre message à envoyer (default %default)")
    parser.add_option("-o", "--username", default=None,
        help="username authentication (default %default)")
    parser.add_option("-p", "--password", default=None,
        help="password authentication (default %default)")
...
amqp_address = options.queue
if(options.topic!="a/topic"):
        amqp_address = 'topic://' + options.topic

try:
    message = "hello" #open("message.txt","r")
    Container(MessageProducer(options.url, amqp_address, options.messages, options.username, options.password, options.payload)).run()
